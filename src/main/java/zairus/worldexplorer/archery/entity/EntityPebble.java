	...
	implements IProjectile
{
        	...
        	try
        	{
        		for (int p_i = 0; p_i < (c_flag ? 35 : 15); p_i++)
        			this.worldObj.spawnParticle(
        					"blockcrack_" + Block.getIdFromBlock(block) + "_0" // + worldObj.getBlockMetadata(this.positionX, this.positionY, this.positionZ)
        					, this.positionX
        					, this.positionY
        					, this.positionZ
        					, 0.0D
        ...
        else
        {
            ...
            if (movingobjectposition != null)
            {
                if (movingobjectposition.entityHit != null)
                {
                    ...
                    {
                        damagesource = (new EntityDamageSourceIndirect("pebble", this, this)).setProjectile();
                    }
                    else
                    {
                        damagesource = (new EntityDamageSourceIndirect("pebble", this, this.shootingEntity)).setProjectile();
                    }
    ...
    public void onCollideWithPlayer(EntityPlayer player)
    {
        if (!this.worldObj.isRemote && this.inGround && this.arrowShake <= 0)
        {
            ...
            if (this.canBePickedUp == 1 && !player.inventory.addItemStackToInventory(new ItemStack(WEArcheryItems.pebble, 1)))
            {
                flag = false;
            }
